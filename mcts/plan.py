...
parser.add_argument("--pp", type=int, help="number of threads", default=20)
parser.add_argument("--nonn", help="Don't use NN", action='store_true' )
...
use_network = False if args.nonn else True
...
print("Planning started. Strategy:", strategy," Parallelisation:", parallelised)
...
elif parallelised: # Create parallel instances for execution on PP
    parallel_instances = args.pp
    job_server = pp.Server()
    state_copies = [deepcopy(root_state)] * parallel_instances
    jobs = [job_server.submit(step,(statecopy, my_oracle, weights, objective, use_network, vn_name, timebudget,), (), ("state","useroracle","mcts",)) for statecopy in state_copies]
