...
import state3
import useroracle2
from useroracle2 import UserStrategy, UserOracle
import mcts2
from state3 import State, MenuState, UserState
from copy import deepcopy
# import plan_simple
# from plan_simple import Newmenu
...
currentmenu =['tomato','chair', 'potato','shoes' , '-', '----', '-','-', '-','-','gloves', 'tomato', 'chair', 'potato', 'shoes' , 'gloves']
freqdist, total_clicks, history = utility.load_click_distribution(currentmenu,
                                                                  "./input/split/" + args.history)  # load from user 2 (CSV file)
...
def step(state, oracle, weights, objective, use_network, network_name, timebudget):
    ...
    tree = mcts2.mcts(oracle, weights, objective, use_network, network_name, time_limit=timebudget)
    node = None
...
for step in bestmenu:
    step[0]=step[0][:6]+ step[0][11:]
    # print(step)
    if step[2]:
        #
        utility.save_menu(step[0], step[2],"output/adaptedmenu/" + "true1" + "2.csv")
        newmenu=step[0]
        sumz = 0
        for m in range(0,5):
            if step[0][m]!="-":
