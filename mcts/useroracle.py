...
class UserOracle(): 
    ...
    def recall(self, target, currentmenu, previousmenu = None):
        ...
        if max_activation < 0.5:
            ...
            for position,activation in sorted(self.activations[target].items(), key=lambda x: x[1], reverse=True):
                ...
                if activation >= 0.5:
                    #print("debug",position,simple_menu, simple_menu[position] )
    ...
    def get_individual_rewards(self,state):
        ...
        currentmenu = state.menu_state.simplified_menu()#state是plan_simple 里的 root_state；plan_simple 里的 root_state = State（state.py里的）menu_state,user_state, exposed=True)
