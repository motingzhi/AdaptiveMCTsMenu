...
class State():
    ...
    def take_adaptation(self, adaptation, update_user = True):        
        ...
        if self.exposed and update_user:
            new_state.user_state.update(menu = self.menu_state.menu)
...
class MenuState():
    ...
    def possible_adaptations(self):
        ...
        #         #print("adaptation",test_adaptation)
        ...
        for i in range (1, 5):
            for j in range (6, 11):
                if self.menu[i] == self.separator and self.menu[j] == self.separator: continue # no point swapping separators
                if len(simple_menu)>10:
                    if abs(i-j)!=6 and self.menu[i] is not self.separator:
                        #print("continue")
                        continue # Limit max swap distance
                test_adaptation = Adaptation([i,j,AdaptationType.SWAP_DOWN, True])
                #print("adaptation",test_adaptation)
                adapted_menu = MenuState(self.adapt_menu(test_adaptation), self.associations)
                if (adapted_menu.simplified_menu() not in seen_menus):
                    seen_menus.append(adapted_menu.simplified_menu())
                    possibleadaptations.append(Adaptation([i,j,AdaptationType.SWAP_DOWN, True]))
                    possibleadaptations.append(Adaptation([i,j,AdaptationType.SWAP_DOWN, False]))
                    #print("swapdown")
        ...
        # for i in range(0, len(self.menu)):
        #     for j in range (0, len(self.menu)):
        #         test_adaptation = Adaptation([i, j, AdaptationType.REMOVE_ITEM, True])
        #         # print("adaptation",test_adaptation)
        #         adapted_menu = MenuState(self.adapt_menu(test_adaptation), self.associations)
        #         if (adapted_menu.simplified_menu() not in seen_menus):
        #             seen_menus.append(adapted_menu.simplified_menu())
        #             possibleadaptations.append(Adaptation([i, j, AdaptationType.REMOVE_ITEM, True]))
        #             possibleadaptations.append(Adaptation([i, j, AdaptationType.REMOVE_ITEM, False]))
        #             print("remove")
    ...
    def adapt_menu(self, adaptation):
        ...
        if adaptation.type == AdaptationType.SWAP_DOWN:
            if adaptation.i < 5 and adaptation.j >5 and adaptation.j<11 :
        ...
        # n=1
        # m=1
        # if adaptation.type == AdaptationType.REMOVE_ITEM:
        #     if new_menu[adaptation.i]!= "tomato":
        #         new_menu[adaptation.i]="----"
        #         n= n+1
        #     else:
        #         m=m+1
        #         print("m!",m)
    ...
    def simplified_menu(self, trailing_separators = False):
        simplified_menu = []
        ...
        if trailing_separators:
           ...
           # print("waha", old_length, new_length, simplified_menu)
...
class UserState():
    ...
    def update(self, menu, number_of_clicks = None):
        ...
        for click in clicks_to_add:
            #print("click!",click ,item_list, self.history)
            if click in item_list:
                self.history.append([click, item_list.index(click)])
            else:
                self.history.append([click,0])
