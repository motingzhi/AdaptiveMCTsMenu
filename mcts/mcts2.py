...
import utility
import os
from useroracle2 import UserOracle
from copy import deepcopy
from adaptation import Adaptation
from state2 import AdaptationType
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'value_network'))
from model import ValueNetwork


# Antti Oulasvirta, Kashyap Todi
...
class mcts():
    ...
    def execute_round(self):
        ...
        if node is not self.root and self.use_network:
            ...
            rewards = self.rollout(node.state, self.oracle)###evaluate part roll out score
            #print("cn",node.state,rewards)
    ...
    def get_reward_predictions(self, node):
        ...
        if node.parent is not None:
            ...
            predictions = self.vn.predict_batch([samples[0]])
    ...
    def expand(self, node):
        ...
        for  adaptation in adaptations:
            ...
            if adaptation not in node.children.keys():
                ...
                #print("now!",adaptation,node.children.keys())
                newNode = TreeNode(node.state.take_adaptation(adaptation), node)
                node.children[adaptation] = newNode
    ...
    def get_best_child(self, node, exploration_const):
        ...
        for child in children:
            ...
            # node_value = child.total_reward / child.num_visits + exploration_const * math.sqrt(
                # math.log(node.num_visits) / child.num_visits)
    ...
    def get_adaptation_probabilities(self, node, exploration_const):
        ...
        for adaptation,child in node.children.items():
            total_reward = sum([a*b for a,b in zip(self.weights, child.total_rewards)])
            node_value = total_reward/child.num_visits + exploration_const * math.sqrt(math.log(node.num_visits) / child.num_visits)
            total_value += node_value
            probability[adaptation] = node_value # Store raw value first

        if total_value == 0.0 : return None
        
        # Normalise the raw values now
        probability = {a: v/total_value for a,v in probability.items()}
        return probability
